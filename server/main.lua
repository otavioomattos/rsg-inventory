...
-- Decay System
local ConvertQuality = function(item)
    ...
    local TimeExtra = ceil((TimeAllowed * DecayRate))
    local decay = 100 - ceil((((os.time() - StartDate) / TimeExtra) * 100))
    local percentDone = decay
...
local function AddItem(source, item, amount, slot, info, created)
    ...
    if not itemInfo and not Player.Offline then
        RSGCore.Functions.Notify(source, 'Alforge', "O item não existe", 'error')
        return false
    end
...
---Use an item from the RSGCore.UsableItems table if a callback is present
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount, created)
    ...
    if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
        ...
        RSGCore.Functions.Notify(source, 'Alforge', Lang:t("notify.missitem"), "error")
    end
end
...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
    ...
    if not ply.state.inv_busy then
        ...
        RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.noaccess"), 'error')
    end
end)
    ...
    elseif itemData.useable then
        if itemData.info.quality > 0 then
            ...
            if itemInfo['delete'] and RemoveItem(src, itemData.name, 1, slot) then
                ...
                RSGCore.Functions.Notify(src, 'Alforge', RSGCore.Shared.Items[itemData.name].label..' estragou e foi removido!', 'error')
            else
                RSGCore.Functions.Notify(src, 'Alforge', 'Você não pode usar este item!', 'error')
            end
        end
    end
end)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
    ...
    if fromInventory == "player" or fromInventory == "hotbar" then
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.missitem"), "error")
        end
    elseif RSGCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            RSGCore.Functions.Notify(src, 'Alforge', "O item não existe", "error")
        end
    elseif RSGCore.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
        ...
        if fromItemData and fromItemData.amount >= fromAmount then
            ...
            RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.itemexist"), "error")
        end
    elseif RSGCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
        ...
        if RSGCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
            if RSGCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
                ...
                if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
                    ...
                    RSGCore.Functions.Notify(src, 'Alforge', itemInfo["label"] .. " bought!", "success")
                    ...
                    RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.notencash"), "error")
                end
            else
                if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
                    itemData.info.quality = itemData.info.quality or 100
                    ...
                    RSGCore.Functions.Notify(src, 'Alforge', "Você não tem dinheiro suficiente..", "error")
                end
            end
        elseif RSGCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
            if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                RSGCore.Functions.Notify(src, 'Alforge', itemInfo["label"] .. " comprado!", "success")
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    if Player == OtherPlayer then return RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.gsitem")) end
    if dist > 2 then return RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.tftgitem")) end
    local item = GetItemBySlot(src, slot)
    if not item then RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.infound")); return end
    if item.name ~= name then RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.iifound")); return end
    ...
    if amount <= item.amount then
        ...
        if RemoveItem(src, item.name, amount, item.slot) then
            if AddItem(target, item.name, amount, false, item.info, item.created) then
                ...
                RSGCore.Functions.Notify(target, 'Alforge', Lang:t("notify.gitemrec")..amount..' '..item.label..Lang:t("notify.gitemfrom")..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
                TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
                TriggerClientEvent('inventory:client:ItemBox',src, RSGCore.Shared.Items[item.name], "remove")
                RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.gitemyg") .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
                ...
                RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.gitinvfull"), "error")
                RSGCore.Functions.Notify(target, 'Alforge', Lang:t("notify.giymif"), "error")
                TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
                TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
            end
        else
            RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.gitydhei"), "error")
        end
    else
        RSGCore.Functions.Notify(src, 'Alforge', Lang:t("notify.gitydhitt"))
    end
end)
...
RSGCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
    ...
    if invType and invId then
        ...
        elseif invType == "stash" then
            ...
            RSGCore.Functions.Notify(source,  'Alforge', Lang:t("notify.navt"), "error")
        end
    else
        RSGCore.Functions.Notify(source,  'Alforge', Lang:t("notify.anfoc"), "error")
    end
end, "admin")
    ...
    if Player then
            if itemData then
                ...
                if AddItem(id, itemData["name"], amount, false, info) then
                    RSGCore.Functions.Notify(source, 'Alforge', Lang:t("notify.yhg") ..GetPlayerName(id).." "..amount.." "..itemData["label"].. "", "success")
                else
                    RSGCore.Functions.Notify(source,  'Alforge', Lang:t("notify.cgitem"), "error")
                end
            else
                RSGCore.Functions.Notify(source,  'Alforge', Lang:t("notify.idne"), "error")
            end
    else
        RSGCore.Functions.Notify(source,  'Alforge', Lang:t("notify.pdne"), "error")
    end
end, "admin")
...
RSGCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    ...
    if Player then
        ...
        RSGCore.Functions.Notify(source, 'Alforge', "Jogador não online", 'error')
    end
end, 'admin')
