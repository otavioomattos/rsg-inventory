...
local function CreateItemDrop(index)
    ...
    SetEntityAsMissionEntity(dropItem, true)
    SetPickupLight(dropItem, true)
...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    ...
    RSGCore.Functions.TriggerCallback('inventory:server:ConvertQuality', function(data)
        ...
        SendNUIMessage(
        {
            ...
            pcash = PlayerData.money.cash,
            pgold = PlayerData.money.gold,
        })
    end, inventory, other)
...
RegisterNUICallback('Notify', function(data, cb)
    RSGCore.Functions.Notify('Alforge', data.message, data.type)
    cb('ok')
end)
...
RegisterNUICallback('combineWithAnim', function(data, cb)
    ...
    RSGCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        StopAnimTask(ped, aDict, aLib, 1.0)
        RSGCore.Functions.Notify('Alforge', Lang:t("notify.failed"), "error")
    end)
    cb('ok')
end)
...
RegisterNUICallback('GiveItem', function(data, cb)
    local players = getNearestPlayers(2)
    local infoplayers 
    local options = {}
    for _, player in ipairs(players) do
       infoplayers = player
       table.insert(options, {label = tostring(player), value = player})
    end
 
    if infoplayers then
        local dialog = lib.inputDialog('GiveItem', {
          {
             label = 'ID', 
             description = 'Selecione o jogador que receberá',
             type = 'select',
             options = options,
             required = true,
          },
        ...
        local playerId = dialog[1]
        TriggerServerEvent("inventory:server:GiveItem", playerId, data.item.name, data.amount, data.slot)
    else
        RSGCore.Functions.Notify("Dar item", Lang:t('notify.nonb'), 'error')
        cb(false)
    end
end)
    ...
    for _, v in ipairs(GetActivePlayers()) do
        if (PlayerPedId() ~= GetPlayerPed( v )) then
            ...
            if distance <= radius then
                table.insert(r, GetPlayerServerId(NetworkGetEntityOwner(GetPlayerPed(v))))
            end
        end
    end
    return r
